 ...
 * kiosk's other camera angles still visibly stuttering at 200ms once the
 * Canon was connected and this panel's live view auto-started): raised to
 * 350ms as the next conservative step. The 200ms figure above was only ever
 * confirmed clean on ONE machine/camera-count combination — this kiosk's
 * exact number of real webcams, USB topology, and CPU are different
 * variables the earlier test never covered, so there is no reason to expect
 * the same number holds everywhere. Still needs this exact kiosk's own
 * hands-on confirmation; if 350ms still isn't enough, the next lever is
 * disabling this panel's auto-start (see the effect below) rather than
 * pushing the interval down further, since slower still beats none.
 */
...
export function TetheredCameraPanel({
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (!cancelled) timer = setTimeout(poll, THERMAL_POLL_MS);
    };
    void poll();
    return () => {
      cancelled = true;
          ...
          <div className="grid grid-cols-2 gap-3 max-w-md">
            ...
            {tetheredLiveViewFrame && (
              <div>
                ...
                <img src={tetheredLiveViewFrame} alt="Khung live view từ máy ảnh qua dây" className="w-full rounded-lg border border-kiosk-border" />
              </div>
            )}
          </div>
        )}

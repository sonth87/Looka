...
/**
 * Evaluates `workflow_versions.config.eligibility.rules[]` (plan §2.2,
 * `workflow-config.schema.ts`'s own `eligibilityRuleSchema` — `expr` is only
 * checked as "non-empty string" at config-save time; REAL parsing/evaluation
 * happens here, the first real caller). `expr-eval` (never `eval()`, plan's
 * own explicit "never eval()" instruction) parses a safe subset of JS-like
 * math/comparison/logical syntax — e.g. `course_year >= 2023 && status ==
 * "ACTIVE"` — against a flat `context` object (roster fields and/or the
 * external API record's own field names, whichever the campaign's
 * `eligibility.mode` put in play — see `CampaignSubjectService.
 * lookupSubject`).
 *

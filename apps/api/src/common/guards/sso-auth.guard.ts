...
export class SsoAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (process.env.ALLOW_UNAUTHENTICATED_ADMIN_DEV === 'true') {
      ...
      if (devMockMatch) {
        ...
        req.user = await this.upsertUser({ user_code: `dev:${localPart}`, email, name: localPart });
        return true;
      }
    }
    ...
    const profile = await this.getProfile(ssoBaseUrl, authorization, req.header('x-refresh-token'));
  ...
  private async getProfile(
    ...
    try {
      ...
      this.logger.warn(`Could not reach SSO backend at ${ssoBaseUrl}: ${(error as Error).message}`);
    ...
    const body = (await profileRes.json().catch(() => null)) as SsoProfileResponse | null;
    if (body) {
      this.profileCache.set(cacheKey, { profile: body, expiresAt: Date.now() + PROFILE_CACHE_TTL_MS });
  ...
  private async upsertUser(ssoUser: NonNullable<SsoProfileResponse['user']>): Promise<AuthenticatedUser> {
    let user = await this.userRepo.findOne({ where: { ssoUserCode: ssoUser.user_code } });
    const adminEmails = this.configService.get<string[]>('security.adminEmails') ?? [];
    const now = new Date();

    if (!user) {
      const anyAdminExists = (await this.userRepo.count({ where: { isAdmin: true } })) > 0;
      const bootstrapAdmin = !anyAdminExists && adminEmails.includes(ssoUser.email.toLowerCase());

...
/**
 * "Duyệt ảnh" detail page (C5, cms-photo-review-plan.md §5.2). Route
 * `/review/:id`, mirroring `CampaignDetail`'s own list→detail navigation
 * style (a route, not local component state). Every action in the bottom
 * bar is disabled with a lock message while `isReviewSetLocked` — the one
 * exception the plan calls out is "Tạo lại ảnh 4x6" while `AUTO_FAILED`
 * (§4/R-Q1).
 */
export function ReviewDetailPage() {
  ...
  if (!set || !id) return <p className="text-gray-500">Đang tải...</p>;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <div className="p-4 rounded-2xl border border-gray-200 bg-white shadow-sm">
          ...
          <div className="grid grid-cols-3 gap-2">
            {set.originals.map((photo) => (
              <button
                key={photo.id}
                type="button"
                onClick={() => photo.viewUrl && setLightboxUrl(photo.viewUrl)}
                disabled={!photo.viewUrl}
                className="aspect-square rounded-lg overflow-hidden bg-gray-50 border border-gray-200 relative"
              >
                {photo.viewUrl ? (
                  <img src={photo.viewUrl} alt={photo.angleLabel ?? photo.stepType ?? photo.cameraRole} className="w-full h-full object-cover" />
                ) : (
                  <span className="text-[10px] text-gray-400 flex items-center justify-center h-full px-1 text-center">Chưa có</span>
                )}
                {photo.fallback && (
                  <span className="absolute bottom-0 inset-x-0 bg-amber-500/90 text-white text-[9px] text-center py-0.5">
                    fallback
                  </span>
                )}
              </button>
            ))}
            {set.originals.length === 0 && <p className="text-xs text-gray-400 col-span-3">Chưa có ảnh gốc.</p>}
          </div>
          <p className="text-xs text-gray-500 mt-2">
            {set.originals.length} ảnh
          ...
          {set.videos.length > 0 && (
            <div className="mt-3 space-y-2">
              <h3 className="text-xs font-medium text-gray-700">Video</h3>
              {set.videos.map((v) => (
                <video key={v.id} controls src={v.viewUrl} className="w-full rounded-lg border border-gray-200" />
              ))}
            </div>
          )}
        </div>

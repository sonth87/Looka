...
| I-Q21 | **Phát hiện 2026-09-14, đọc lại source thật của file-service** (`D:\Work\file-service\projects\`, repo Go riêng): mọi file Looka upload với `visibility: 'private'` (ảnh/video chụp SV, variant đã retouch, avatar CMS, roster xlsx, JSON sidecar photo-review) **không đọc được vĩnh viễn** qua `issueDownloadLink`/`issueViewLink`. Lý do đã lần theo tận `authz.Decide()` (`fs-engine/authz/authz.go`): `FsClient` xác thực thuần `X-API-Key` (`tenant.PrincipalForApp` → `Roles:["app"]`, không phải `admin`), không bao giờ gửi `X-Owner-User-Id` → `owner_user_id` luôn `null`; `isOwner()` không bao giờ khớp `null`; file `private` + không phải chủ sở hữu → `Decide` luôn từ chối `404 NOT_FOUND`, kể cả với token share-link hợp lệ do `issueDownloadLink` cấp (`download/service.go` vẫn gọi `LoadFileForAction`→`Decide` sau bước kiểm scope). Không thể sửa bằng cách gán `owner_user_id` khớp `viewerId` lúc đọc: `viewerId` truyền vào `issueViewLink` là **nhãn cố định theo module** (`'card-template'`, `'print-package'`, `'students-gallery'`...), không phải 1 identity ổn định — cùng 1 file bị nhiều module đọc lại với nhãn khác nhau. | **ĐÃ SỬA 2026-09-14** (quyết định user, áp dụng cho cả nhóm ảnh sinh trắc học sau khi cảnh báo rủi ro rõ ràng): đổi toàn bộ 8 điểm upload private → `'public'` (`photo.service.ts` x2, `session-video.service.ts`, `variant-upload-worker.service.ts`, `photo-review.service.ts` sidecar JSON, `campaign-subject.service.ts` x2 roster/error-report, `user.command.controller.ts` avatar). An ninh thực tế **không đổi**: Looka chưa từng dựa vào ACL riêng của file-service — mọi lượt mint link đều đi qua `@RequirePermission` guard của chính Looka trước; file-service `visibility:'public'` chỉ nghĩa "đọc được nếu cùng tenant + cầm API key hoặc share-token hợp lệ do Looka cấp", không phải công khai ra ngoài. Verify: build sạch, lint không phát sinh lỗi mới (xác nhận bằng `git stash` chạy lại eslint trên bản gốc — cùng 12 lỗi y hệt, đều là lỗi có sẵn không liên quan), 102 test pass (109 skip vì cần DB thật, đúng quy ước cũ). **Chưa làm, không thuộc phạm vi hôm nay**: 3 concern ban đầu khác (Idempotency-Key lặp lại trên mọi chunk, X-Content-SHA256 gửi thừa mỗi chunk, header thừa trên request probe offset) đã xác nhận **không phải lỗi** — đọc thẳng `upload.go`/`direct.go`/tài liệu chính thức `LUONG-UPLOAD-DOWNLOAD-CHI-TIET.md` xác nhận chế độ chunked không hề đọc `Idempotency-Key`, và các header khác chỉ được đọc đúng 1 lần lúc mở phiên. | — |

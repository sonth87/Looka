...
once) → KYC enrollment (3.7, still blocked on an external protocol either
way). 3.1's video recording still needs a real-environment test pass
(`MediaRecorder` in an actual browser/Electron window — no display/simulator
was available here) before it's trusted in production; it was only verified
by static review and the DB layer's real tests.

import { CaptureTriggerConfig, GestureState, GestureType } from '@face/core';
...
/**
 * CaptureTriggerEvaluator controls WHEN a capture fires based on the selected trigger mode.
 * - AUTO: fires when faceStabilityProgress >= 1.0
 * - MANUAL: fires after a valid hand gesture is held for 500ms while face is ready
 * - OFF: fires when the user explicitly presses the shutter button
 */
export class CaptureTriggerEvaluator {
  ...
  public evaluate(params: {
    ...
    // Mode 1: AUTO (Hold pose for autoHoldMs — stability managed by StabilityTracker)
    if (mode === 'AUTO') {
